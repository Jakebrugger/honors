...
t_dum_builder <- function(year){
  ...
  dummy_list <- rep(0, times = 18)
  dummy_list[year] <- 1
  return_list <- rep(dummy_list, times = 55)
  return(return_list)
}
...
id_list <-  rep(1, times = 18)
for(i in 2:55){
  id_list = c(id_list, rep(i, times = 18))
}


time_list <- rep((1:18), times = 55)
time_list
...
tdum17<- t_dum_builder(17)
tdum18<- t_dum_builder(18)
...
fixed_effect <- plm(Density ~ fin_ratio, data = pdata, model = 'within', index=c("id","t"))
fixed_effect_with_dep <- plm(Density ~ fin_ratio, data = pdata_with_dep, model = 'within', index=c("id","t"))
